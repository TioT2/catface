...
typedef struct __CfTirVariableReference {
    CfTirBlock * block;         ///< TIR block
    uint32_t     variableIndex; ///< local variable index
} CfTirVariableReference;
...
typedef enum CfTirExpressionType_ {
    ...
    CF_TIR_EXPRESSION_BINARY_OPERATOR, ///< binary operator
} CfTirExpressionType;
...
typedef enum CfTirBuildingStatus_ {
    CF_TIR_BUILDING_STATUS_OK,
    CF_TIR_BUILDING_STATUS_INTERNAL_ERROR,
} CfTirBuildingStatus;
